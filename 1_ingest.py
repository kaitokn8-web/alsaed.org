...
def read_pdf(path, book_name):
    ...
    with fitz.open(path) as pdf:
        for i, page in enumerate(pdf):
            # استخراج النص مع الحفاظ على اتجاه العربية
            blocks = page.get_text("blocks")
            blocks.sort(key=lambda b: (-b[1], b[0]))  # ترتيب من أعلى لأسفل
...
def read_word(path, book_name):
    ...
    # نقسم النص عند أرقام الصفحات مثل (٥٤٥/٢) أو (545/2)
    page_pattern = re.compile(r'\((\d+[\u0660-\u0669]*)/(\d+[\u0660-\u0669]*)\)')
    
    chunks_text = page_pattern.split(full_text)
